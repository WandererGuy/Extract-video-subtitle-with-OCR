...
OCR_MAPPING_FOLDER = "ocr_mapping"
...
def extract_frames_with_timestamps(video_path: str, skip: int = 15):
    ...
    os.makedirs(os.path.join(STATIC_FOLDER, VIDEO_NAME), exist_ok = True)
    OCR_MAPPING_FILE = os.path.join(OCR_MAPPING_FOLDER, VIDEO_NAME + ".yaml")
    ...
    while True:
        ...
        if frame_idx % skip == 0:
            ...
            if compare_ocr_output(ocr_output, last_ocr_output):
                ...
                output_path_segment = os.path.join(STATIC_FOLDER, VIDEO_NAME, str(uuid.uuid4()) + ".mp4")
                """
                ...
                """
                try: # in case different ocr switch happen at 0.1 or any < 0.2 seconds
                    ...
                    with open (OCR_MAPPING_FILE, "a", encoding = "utf-8") as f:
                        text = fix_path(str(output_path_segment)) + ":" + "\n" + \
...
if __name__ == "__main__":
    folder = r"D:\WORK\Speech2text_dan_toc\all_video_dan_toc"
